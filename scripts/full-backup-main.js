...
Hooks.on("preUpdateActor", (actor, updateData, options, userId) => {
  ...
  // If we're updating system data
  if (updateData.system) {
    // Calculate DR from equipped armor
    const calculatedDR = calculateActorDR(actor);
    
    // Check if this update includes a manualDROverride change
    const isUpdatingManualDR = hasProperty(updateData.system, "attributes.manualDROverride");
    
    // Get current manual override value or the one being set in this update
    let manualDR;
    if (isUpdatingManualDR) {
      manualDR = getProperty(updateData.system, "attributes.manualDROverride");
    } else {
      manualDR = actor.system.attributes?.manualDROverride;
    }
    
    // Use manual DR if set, otherwise use calculated
    const finalDR = (manualDR !== undefined && manualDR !== null) ? manualDR : calculatedDR;
    
    // Always set the damageReduction value when updating the actor
    setProperty(updateData.system, "attributes.damageReduction", finalDR);
  }
});

// Hook into item updates (specifically when armor is equipped/unequipped)
Hooks.on("updateItem", async (item, updateData, options, userId) => {
  ...
  if (item.type === "armor" && hasProperty(updateData, "system.equipped")) {
    ...
    console.log("Armor equipped status changed:", {
      armor: item.name,
      equipped: updateData.system.equipped,
      actor: actor.name
    });
    
    // Force a recalculation of DR
    const calculatedDR = calculateActorDR(actor);
    
    // Update the actor's DR
    await actor.update({
      "system.attributes.damageReduction": calculatedDR
    });
    
    // Rerender the actor sheet
...
// Hook into actor sheet preparation
Hooks.on("prepareyzecoriolisActorSheetData", (app, data) => {
  if (!game.settings.get(MODULE_ID, "enableCombatReloaded")) return;
  
  // Ensure DR is calculated correctly
  const calculatedDR = calculateActorDR(data.actor);
  
  // Get current manual override value
  const manualDR = data.actor.system.attributes?.manualDROverride;
  
  // Use manual DR if set, otherwise use calculated
  const finalDR = (manualDR !== undefined && manualDR !== null) ? manualDR : calculatedDR;
  
  // Update the sheet data
  data.actor.system.attributes.damageReduction = finalDR;
});
...
function calculateActorDR(actor) {
  ...
  console.log(`Calculated DR for actor ${actor.name}:`, totalDR);
  return totalDR;
}
...
function addManualDRToActorSheet(app, html, data) {
  // Get current values - calculate fresh from equipped armor
  const calculatedDR = calculateActorDR(app.actor);
  const manualDR = app.actor.system.attributes?.manualDROverride;
  const finalDR = (manualDR !== undefined && manualDR !== null) ? manualDR : calculatedDR;
  
  // Update the actor if needed to ensure DR is correct
  if (app.actor.system.attributes.damageReduction !== finalDR) {
    app.actor.update({
      "system.attributes.damageReduction": finalDR
    });
  }
  
  // Create the DR entry with a simple numeric display for debugging
  const drEntry = `
...
function onClickDRBarSegment(event) {
  ...
  const actorId = $(event.currentTarget).closest('.sheet.actor').data('actorId');
  if (actorId) {
    const actor = game.actors.get(actorId);
    if (actor) {
      // Update the actor's manualDROverride
      actor.update({"system.attributes.manualDROverride": newValue});
    }
  }
}
// // Import these from the core system
// import { onHoverBarSegmentIn, onHoverBarOut } from './module/actor/databar.js';
...
Hooks.on("preCreateActor", (document, data, options, userId) => {
  ...
  // Initialize both DR properties
  document.updateSource({
    "system.attributes.damageReduction": 0,
    "system.attributes.manualDROverride": null  // Using null so calculated value is used by default
  });
});
...
Hooks.on("renderyzecoriolisItemSheet", (app, html, data) => {
  ...
  // Find all Armor Penetration fields
